...
class Suite:
    ...
    def interactions(self):
        ...
        expect(p.locator('.stack-scene')).to_be_visible()
        expect(p.locator('.stack-svg')).to_have_count(1)
        expect(p.locator('.stack-caption')).to_contain_text('Applications')
        self.mark('Homepage layered stack illustration')
        self.load('services/')
        expect(p.locator('.services-checker-card')).to_have_count(6)
        expect(p.locator('.services-logo-blocks')).to_contain_text('Yocto engineering')
        self.mark('Services page and checkerboard cards')
        self.load()
